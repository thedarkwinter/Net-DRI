...
xn--rhqv96g xn--g2xx48c xn--nyqy26a best uno safety pharmacy nyc jetzt taipei qpon moe buzz ceo htc club kyoto whoswho osaka
...
=cut
 ...
 return {
     bep_type => 1, # dedicated registy
     tlds => ['xn--rhqv96g','xn--g2xx48c','xn--nyqy26a','best','uno','safety','pharmacy','nyc','jetzt','taipei','qpon','moe','buzz','ceo','htc','club','kyoto','whoswho','osaka'],
     transport_protocol_default => ['Net::DRI::Transport::Socket',{},'Net::DRI::Protocol::EPP::Extensions::NEUSTAR',{}],
     whois_server => (defined $tld && $tld =~ m/\w+/ ? 'whois.nic.' . $tld : undef),
   } if $bep eq 'neustar';
...
dpml.pub actor airforce army attorney auction band consulting dance degree democrat dentist engineer forsale futbol gives haus immobilien kaufen lawyer market moda mortgage navy news ninja pub rehab republican reviews rip rocks sale social software vet video
...
=cut
 ...
 return {
     bep_type => 2, # shared registry
     tlds => ['dpml.pub','actor','airforce','army','attorney','auction','band','consulting','dance','degree','democrat','dentist','engineer','forsale','futbol','gives','haus','immobilien','kaufen','lawyer','market','moda','mortgage','navy','news','ninja','pub','rehab','republican','reviews','rip','rocks','sale','social','software','vet','video'],
     transport_protocol_default => ['Net::DRI::Transport::Socket',{},'Net::DRI::Protocol::EPP::Extensions::UNITEDTLD',{}],
...
art audio auto blackfriday cars christmas click deal diet family flowers free game garden gift guitars help hiphop home hosting inc juegos link lol love mom photo pics property racing realestate sale save sexy shopping store tattoo team tech video yoga
...
=cut
 ...
 return {
     bep_type => 2, # shared registry
     tlds => ['art','audio','auto','blackfriday','cars','christmas','click','deal','diet','family','flowers','free','game','garden','gift','guitars','help','hiphop','home','hosting','inc','juegos','link','lol','love','mom','photo','pics','property','racing','realestate','sale','save','sexy','shopping','store','tattoo','team','tech','video','yoga'],
