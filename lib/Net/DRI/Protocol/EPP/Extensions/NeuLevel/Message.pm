## Domain Registry Interface, EPP Message for NeuLevel
##
...
sub parse
{
 ...
 foreach my $r (@r)
 {
  foreach my $rinfo (@$r)
  {
   next unless $rinfo->{from} eq 'eppcom:value' && $rinfo->{type} eq 'rawxml' && $rinfo->{message}=~m!^<value><text>(.+?)</text></value>$!;
   $rinfo->{message}=$1;
   $rinfo->{from}='eppcom';
   $rinfo->{type}='text';
  }
...
Net::DRI::Protocol::EPP::Extensions::NeuLevel::Message - EPP NeuLevel Message for Net::DRI
