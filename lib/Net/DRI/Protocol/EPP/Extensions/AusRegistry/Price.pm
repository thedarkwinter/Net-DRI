...
sub setup
{
 ...
 state $ns = { 'price' => 'urn:ar:params:xml:ns:price-1.2' };
 $po->ns($ns);
 return;
}
...
sub check_build
{
 ...
 $epp->message()->command_extension('price', ['check', Net::DRI::Protocol::EPP::Util::build_period($rp->{price}, 'price')]) if Net::DRI::Util::is_class($rp->{price}, 'DateTime::Duration');
...
sub _build
{
 ...
 $epp->message()->command_extension('price', [$topname, ['ack', $rp->{price} ne 'ack' ? ['price', 0+$rp->{price}] : ()]]);
 return;
}
