...
sub create {
 ...
 return unless @n;
 $epp->message()->command_extension('domain-nom-ext', ['create', @n]);
 return;
}
...
sub update {
 ...
 my @n = domain_nom_ext($rd);
 return unless @n;
 $epp->message()->command_extension('domain-nom-ext', ['update', @n]);
 return;
}
...
sub list
{
 ...
 $mes->command(['info','l:list',sprintf('xmlns:l="%s"',$mes->ns('std-list'))]);
 my @d;
