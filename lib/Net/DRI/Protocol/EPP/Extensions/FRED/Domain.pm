...
sub info_parse {
  ...
  while ($c) {
    ...
    if ($name eq 'name') {
      ...
      $cs->set($po->create_local_object('contact')->srid($c->getFirstChild()->getData()),$1);
    } elsif ($name eq 'ns') {
      $ns = Net::DRI::Data::Hosts->new() if (!$ns);
      $ns->add($c->getFirstChild()->getData());
    } elsif ($name eq 'nsset') {
      $rinfo->{domain}->{$oname}->{nsset} = $c->getFirstChild()->getData();
    } elsif ($name eq 'host') {
...
sub create {
  ...
  ## AuthInfo
  Net::DRI::Exception::usererr_insufficient_parameters('authInfo is mandatory')
...
sub update {
  ...
  Net::DRI::Exception::usererr_invalid_parameters
    ($todo . ' must be a Net::DRI::Data::Changes object')
...
sub transfer_request {
  my ($epp, $name, $rd) = @_;
  my $mes = $epp->message();
  my @d = build_command_transfer($mes,$name,'request',$rd);
  $mes->command_body(\@d);
  return;
}
