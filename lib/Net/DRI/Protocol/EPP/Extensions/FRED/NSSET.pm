...
sub build_contacts {
  ...
  foreach my $type ($cs->types()) {
    push(@a, map { ['nsset:' . $type, $_->srid()] } $cs->get($type));
  }
  ...
  foreach my $cd ($chkdata->getElementsByTagNameNS($ns, 'cd')) {
    ...
    while ($c) {
      ...
      if ($n eq 'id') {
        ...
        $rinfo->{nsset}->{$nsset}->{exist} = 1 - Net::DRI::Util::xml_parse_boolean
            ($c->getAttribute('avail'));
        $rinfo->{nsset}->{$nsset}->{action} = 'check';
      }
...
sub info_parse {
  ...
  while ($c) {
    ...
    if ($name eq 'id') {
      ...
      $rinfo->{nsset}->{$oname}->{roid} = $c->getFirstChild()->getData();
    } elsif ($name eq 'reportlevel') {
      $rinfo->{nsset}->{$oname}->{reportlevel} = int($c->getFirstChild()->getData());
    } elsif ($name eq 'status') {
      push(@s,Net::DRI::Protocol::EPP::Util::parse_node_status($c));
    } elsif ($name eq 'authInfo') {
      $rinfo->{nsset}->{$oname}->{auth} = { pw => $c->getFirstChild()->getData() };
    } elsif ($name =~ /^((?:c[lr]|tr|up)ID)$/) {
      $rinfo->{nsset}->{$oname}->{$1} = $c->getFirstChild()->getData();
    } elsif ($name =~ /^((?:c[lr]|tr|up)Date)$/) {
      $rinfo->{nsset}->{$oname}->{$1} = $po->parse_iso8601($c->textContent());
    } elsif ($name eq 'ns') {
      my ($hostname,@v4,@v6);
      foreach my $xname ($c->getElementsByTagNameNS(ns($mes), 'name')) {
        $hostname = $xname->getFirstChild()->getData();
      }
      foreach my $xaddr ($c->getElementsByTagNameNS(ns($mes), 'addr')) {
      ...
      $cs->add($po->create_local_object('contact')->srid($c->getFirstChild()->getData()), $name);
    }
  } continue {
		$c = $c->getNextSibling();
	}
...
sub transfer_query {
  my ($epp, $name, $rd) = @_;
  my $mes = $epp->message();
  my @d = build_command($epp, $mes, ['transfer', {'op' => 'query'}], $name);
...
sub transfer_request {
  ...
  my @d = build_command($epp, $mes, ['transfer', {'op' => 'request'}],$name);
  push(@d, build_authinfo($rd->{auth})) if Net::DRI::Util::has_auth($rd);
...
sub transfer_cancel {
  ...
  my @d = build_command($epp, $mes, ['transfer', {'op' => 'cancel'}], $name);
  push(@d, build_authinfo($rd->{auth})) if Net::DRI::Util::has_auth($rd);
...
sub update {
  ...
  if ((grep { ! /^(?:ns|contact|auth|reportlevel)$/ } $todo->types())) {
    Net::DRI::Exception->die(0, 'protocol/EPP', 11, 'Only ns/contact add/del ' .
		'and auth/reportlevel set available for NSSET');
  }
