...
sub _build_price_query
{
 ...
 my $fee_version;
 $fee_version = $ndr->protocol()->{fee_version} if exists $ndr->protocol()->{fee_version};
 $fee_version = $ndr->protocol()->{brown_fee_version} if exists $ndr->protocol()->{brown_fee_version};
...
sub domain_check_claims
{
 ...
 if (defined $rd && exists $rd->{phase} && lc($rd->{phase}) ne 'claims')
 {
  ...
  # MAM/KNet   (Knet seems to work either way - but rather put it here)
  $lp->{sub_phase} = $rd->{phase} if ($bep =~ m/^(?:mam|knet)/);
