...
$dri->add_registry('HostmasterUA');
$dri->target('HostmasterUA')->add_current_profile('p1','epp',{f_send=>\&mysend,f_recv=>\&myrecv});
print $@->as_string() if $@;
...
is_string($R1,$E1.'<command><create><domain:create xmlns:domain="http://hostmaster.ua/epp/domain-1.1"><domain:name>example123.ua</domain:name><domain:period unit="y">5</domain:period><domain:registrant>reg1</domain:registrant><domain:contact type="admin">adm1</domain:contact><domain:contact type="billing">bil1</domain:contact><domain:contact type="tech">tec1</domain:contact><domain:authInfo><domain:pw>2fooBAR</domain:pw></domain:authInfo></domain:create></create><extension><uaepp:create xmlns:uaepp="http://hostmaster.ua/epp/uaepp-1.1"><uaepp:license>abc123</uaepp:license></uaepp:create></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_create build');
is($rc->is_success(),1,'domain_create is_success');

## Host delete with no confirm key
$R2='';
$rc=$dri->host_delete('ns1.example123.ua');
is_string($R1,$E1.'<command><delete><host:delete xmlns:host="http://hostmaster.ua/epp/host-1.1"><host:name>ns1.example123.ua</host:name></host:delete></delete><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_create build');

## Host delete with confirm key
$R2='';
$rc=$dri->host_delete('ns1.example123.ua', { confirm => 'yes'});
is_string($R1,$E1.'<command><delete><host:delete xmlns:host="http://hostmaster.ua/epp/host-1.1"><host:name>ns1.example123.ua</host:name></host:delete></delete><extension><uaepp:delete xmlns:uaepp="http://hostmaster.ua/epp/uaepp-1.1"><uaepp:deleteNS confirm="yes"/></uaepp:delete></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_create build');
