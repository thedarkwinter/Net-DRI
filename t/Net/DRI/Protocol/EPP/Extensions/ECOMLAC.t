...
is($R1,'<?xml version="1.0" encoding="UTF-8" standalone="no"?><epp xmlns="urn:ietf:params:xml:ns:epp-1.0"><command><check><contact:check xmlns:contact="urn:ietf:params:xml:ns:contact-1.0"><contact:id>conexample</contact:id></contact:check></check><clTRID>ABC-12345</clTRID></command></epp>','contact_check build');
...
is($R1,'<?xml version="1.0" encoding="UTF-8" standalone="no"?><epp xmlns="urn:ietf:params:xml:ns:epp-1.0"><command><info><contact:info xmlns:contact="urn:ietf:params:xml:ns:contact-1.0"><contact:id>conexample</contact:id><contact:authInfo><contact:pw>2fooBAR</contact:pw></contact:authInfo></contact:info></info><clTRID>ABC-12345</clTRID></command></epp>','contact_info build');
...
is_string($R1,$E1.'<command><update><contact:update xmlns:contact="urn:ietf:params:xml:ns:contact-1.0"><contact:id>conexample</contact:id><contact:chg><contact:postalInfo type="loc"><contact:name>Example Name</contact:name><contact:addr><contact:street>Example Street 1</contact:street><contact:street>Example Street 2</contact:street><contact:street/><contact:city>Example City</contact:city><contact:sp>Example State</contact:sp><contact:pc>012345</contact:pc><contact:cc>MX</contact:cc></contact:addr></contact:postalInfo><contact:voice>+52.0123456789</contact:voice><contact:email>example@example.lat</contact:email></contact:chg></contact:update></update><clTRID>ABC-12345</clTRID></command>'.$E2,'contact_update build');
is($rc->is_success(),1,'contact_update is_success');

## contact:delete
$R2=$E1.'<response>'.r().$TRID.'</response>'.$E2;
$rc=$dri->contact_delete($dri->local_object('contact')->srid('conexample'));
is_string($R1,'<?xml version="1.0" encoding="UTF-8" standalone="no"?><epp xmlns="urn:ietf:params:xml:ns:epp-1.0"><command><delete><contact:delete xmlns:contact="urn:ietf:params:xml:ns:contact-1.0"><contact:id>conexample</contact:id></contact:delete></delete><clTRID>ABC-12345</clTRID></command></epp>','contact_delete build');
is($rc->is_success(),1,'contact_delete is_success');

## host:create
$R2='';
$rc=$dri->host_create($dri->local_object('hosts')->add('ns.example.lat',['192.0.2.2'],[],1));
is_string($R1,$E1.'<command><create><host:create xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns.example.lat</host:name><host:addr ip="v4">192.0.2.2</host:addr></host:create></create><clTRID>ABC-12345</clTRID></command>'.$E2,'host_create build');
...
is_string($R1,$E1.'<command><check><host:check xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns.example.lat</host:name></host:check></check><clTRID>ABC-12345</clTRID></command>'.$E2,'host_check build');
is($rc->is_success(),1,'host_check is_success');
# multi
$rc=$dri->host_check('ns1.example.lat','ns2.example.lat','ns3.example.lat');
is_string($R1,$E1.'<command><check><host:check xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns1.example.lat</host:name><host:name>ns2.example.lat</host:name><host:name>ns3.example.lat</host:name></host:check></check><clTRID>ABC-12345</clTRID></command>'.$E2,'host_check multi build');
is($rc->is_success(),1,'host_check multi is_success');

## host:info
$R2='';
$rc=$dri->host_info('ns.example.com');
is_string($R1,$E1.'<command><info><host:info xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns.example.com</host:name></host:info></info><clTRID>ABC-12345</clTRID></command>'.$E2,'host_info build');
...
is_string($R1,$E1.'<command><update><host:update xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns.example.lat</host:name><host:add><host:addr ip="v4">192.168.100.2</host:addr></host:add><host:rem><host:addr ip="v4">192.168.100.3</host:addr></host:rem></host:update></update><clTRID>ABC-12345</clTRID></command>'.$E2,'host_update build');
is($rc->is_success(),1,'host_update is_success');

## host:delete
$R2=$E1.'<response>'.r().$TRID.'</response>'.$E2;
$rc=$dri->host_delete('ns.example.com');
is_string($R1,$E1.'<command><delete><host:delete xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns.example.com</host:name></host:delete></delete><clTRID>ABC-12345</clTRID></command>'.$E2,'host_delete build');
...
is_string($R1,$E1.'<command><check><domain:check xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>example-available.lat</domain:name><domain:name>example-reserved.lat</domain:name><domain:name>example-in-use.lat</domain:name></domain:check></check><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_check multi build');
is($rc->is_success(),1,'domain_check multi is_success');

## domain:info
$R2='';
$rc=$dri->domain_info('example.lat');
is_string($R1,$E1.'<command><info><domain:info xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name hosts="all">example.lat</domain:name></domain:info></info><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_info build');
...
is_string($R1,$E1.'<command><update><domain:update xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>example.lat</domain:name><domain:add><domain:status s="clientHold"/></domain:add></domain:update></update><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_update build');
is($rc->is_success(),1,'domain_update is_success');

## domain:delete
$R2='';
$rc=$dri->domain_delete('example.lat');
is_string($R1,$E1.'<command><delete><domain:delete xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>example.lat</domain:name></domain:delete></delete><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_delete build');
...
is_string($R1,$E1.'<command><renew><domain:renew xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>example.lat</domain:name><domain:curExpDate>2011-08-27</domain:curExpDate><domain:period unit="y">1</domain:period></domain:renew></renew><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_renew build');
...
is_string($R1,$E1.'<command><transfer op="request"><domain:transfer xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>example.lat</domain:name><domain:authInfo><domain:pw>2fooBAR</domain:pw></domain:authInfo></domain:transfer></transfer><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_transfer_start build');
...
is_string($R1,$E1.'<command><transfer op="query"><domain:transfer xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>example.lat</domain:name><domain:authInfo><domain:pw>2fooBAR</domain:pw></domain:authInfo></domain:transfer></transfer><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_transfer_query build');
is($rc->is_success(),1,'domain_transfer_query is_success');

## rar:info (check the balance through EPP)
$R2=$E1.'<response><result code="1000"><msg>Command completed successfully</msg></result><msgQ count="34" id="3418"/><resData><rar:infData xmlns:rar="http://www.nic.mx/rar-1.0"><rar:id>rar_registrar</rar:id><rar:roid>RAR_REGISTRAR-LAT</rar:roid><rar:name>Registrar</rar:name><rar:balance>81.10</rar:balance></rar:infData></resData>'.$TRID.'</response>'.$E2;
...
$R2=$E1.'<response>'.r().'<resData><domain:creData xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>idnexample.lat</domain:name><domain:crDate>2010-08-10T15:38:26.623854Z</domain:crDate><domain:exDate>2012-08-10T15:38:26.623854Z</domain:exDate></domain:creData></resData>'.$TRID.'</response>'.$E2;
$rc=$dri->domain_create('idnexample.lat',{pure_create=>1,auth=>{pw=>'2fooBAR'},'idn' => $idn});
is_string($R1,$E1.'<command><create><domain:create xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>idnexample.lat</domain:name><domain:authInfo><domain:pw>2fooBAR</domain:pw></domain:authInfo></domain:create></create><extension><idn:create xmlns:idn="http://www.nic.lat/nicmx-idn-1.0"><idn:lang>ES</idn:lang></idn:create></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_create build_xml');
is($dri->get_info('action'),'create','domain_create get_info(action)');
## domain_check
$R2=$E1.'<response>'.r().'<resData><domain:chkData xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:cd><domain:name avail="1">idn-check-example.lat</domain:name></domain:cd></domain:chkData></resData><extension><idn:checkResData xmlns:idn="http://www.nic.lat/nicmx-idn-1.0"><idn:name>foobar_name</idn:name><idn:base>foobar_base</idn:base><idn:lang>es</idn:lang></idn:checkResData></extension>'.$TRID.'</response>'.$E2;
$rc=$dri->domain_check('idn-check-example.lat');
is_string($R1,$E1.'<command><check><domain:check xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>idn-check-example.lat</domain:name></domain:check></check><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_check build_xml');
is($rc->is_success(),1,'domain_check is_success');
...
## domain_info
$R2=$E1.'<response>'.r().'<resData><domain:infData xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>idn-info-example.lat</domain:name><domain:crDate>2010-08-10T15:38:26.623854Z</domain:crDate><domain:exDate>2012-08-10T15:38:26.623854Z</domain:exDate></domain:infData></resData><extension><idn:infoResData xmlns:idn="http://www.nic.lat/nicmx-idn-1.0"><idn:lang>fr</idn:lang></idn:infoResData></extension>'.$TRID.'</response>'.$E2;
