...
$R2=$E1.'<response>'.r().'<resData><domain:creData xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name>test-5btc8khh-1.ro</domain:name><domain:crDate>2012-09-19T08:08:20Z</domain:crDate></domain:creData></resData>'.$TRID.'</response>'.$E2;
...
$R2=$E1.'<response>'.r().'<resData><domain:creData xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name>test-gbdppjee-ăîșțâ.ro</domain:name><domain:crDate>2015-05-27T08:24:52Z</domain:crDate></domain:creData></resData><extension><idn:mapping xmlns:idn="http://www.rotld.ro/xml/epp/idn-1.0"><idn:name><idn:ace>xn--test-gbdppjee--ohb5qsnr6odb.ro</idn:ace><idn:unicode>test-gbdppjee-ăîșțâ.ro</idn:unicode></idn:name></idn:mapping></extension>'.$TRID.'</response>'.$E2;
$cs=$dri->local_object('contactset');
...
$rc=$dri->domain_update('test-6lijl3yd.ro',$toc);
is_string($R1,$E1.'<command><update><domain:update xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name>test-6lijl3yd.ro</domain:name></domain:update></update><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_update add_hosts (empty) + activation build_xml');
...
$R2=$E1.'<response>'.r().'<resData><domain:renData xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name>test-5btc8khh-6.ro</domain:name><domain:exDate>2014-08-11T15:43:26Z</domain:exDate></domain:renData></resData>'.$TRID.'</response>'.$E2;
...
is_string($R1,$E1.'<command><renew><domain:renew xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name>test-5btc8khh-6.ro</domain:name><domain:curExpDate>2015-05-01</domain:curExpDate><domain:period unit="y">1</domain:period></domain:renew></renew><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_renew build_xml');
...
is_string($R1,$E1.'<command><info><domain:info xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name hosts="all">xn--test-gbdppjee--ohb5qsnr6odb.ro</domain:name></domain:info></info><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_info idn build_xml');
...
### 2.2 Domain Check
$R2=$E1.'<response>'.r().'<resData><domain:chkData xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:cd><domain:name avail="1">romatime.ro</domain:name></domain:cd></domain:chkData></resData><extension><idn:mapping xmlns:idn="http://www.rotld.ro/xml/epp/idn-1.0" /><rotld:ext xmlns:rotld="http://www.rotld.ro/xml/epp/rotld-1.0"><rotld:check_renew_availability renewable="0"><rotld:reason>OPERATION NOT SUPPORTED FOR YOUR REGISTRAR TYPE</rotld:reason></rotld:check_renew_availability></rotld:ext></extension>'.$TRID.'</response>'.$E2;
$rc = $dri->domain_check('romatime.ro');
is_string($R1,$E1.'<command><check><domain:check xmlns:domain="http://www.rotld.ro/xml/epp/domain-1.0"><domain:name>romatime.ro</domain:name></domain:check></check><clTRID>ABC-12345</clTRID></command>'.$E2,'domain_check dna build_xml');
...
#is_string($R1,$E1.'<command><create><host:create xmlns:host="urn:ietf:params:xml:ns:host-1.0"><host:name>ns.1.test-t4s3zmg.ro</host:name><host:addr ip="v4">1.2.3.4</host:addr><host:addr ip="v6">2a03:5e80:0:4::133e</host:addr></host:create></create><clTRID>ABC-12345</clTRID></command>'.$E2,'host_create build_xml');
