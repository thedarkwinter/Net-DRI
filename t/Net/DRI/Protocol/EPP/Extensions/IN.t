...
$dri->add_current_registry('Afilias::IN');
...
my $drd = $dri->driver();
is_deeply( [$drd->transport_protocol_default('epp')],['Net::DRI::Transport::Socket',{'ssl_version'=>'TLSv12', 'ssl_cipher_list' => undef},'Net::DRI::Protocol::EPP::Extensions::IN',{}],'IN - epp transport_protocol_default');
$R2='';
...
## fee-check-class
$R2=$E1.'<response>'.r().'<resData><domain:chkData xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:cd><domain:name avail="1">crrc.in</domain:name></domain:cd></domain:chkData></resData><extension><fee:chkData xmlns:fee="urn:ietf:params:xml:ns:fee-0.8" xsi:schemaLocation="urn:ietf:params:xml:ns:fee-0.8 fee-0.8.xsd"><fee:cd><fee:name>crrc.in</fee:name><fee:currency>INR</fee:currency><fee:command>create</fee:command><fee:period unit="y">1</fee:period><fee:fee applied="delayed" description="Registration fee 0.8" grace-period="P5D" refundable="1">100.00</fee:fee><fee:class>premium</fee:class></fee:cd></fee:chkData>    </extension>'.$TRID.'</response>'.$E2;
$rc=$dri->domain_check('crrc.in',{fee=>{action=>'create'}});
is_string($R1,$E1.'<command><check><domain:check xmlns:domain="urn:ietf:params:xml:ns:domain-1.0" xsi:schemaLocation="urn:ietf:params:xml:ns:domain-1.0 domain-1.0.xsd"><domain:name>crrc.in</domain:name></domain:check></check><extension><fee:check xmlns:fee="urn:ietf:params:xml:ns:fee-0.8" xsi:schemaLocation="urn:ietf:params:xml:ns:fee-0.8 fee-0.8.xsd"><fee:domain><fee:name>crrc.in</fee:name><fee:command>create</fee:command></fee:domain></fee:check></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'fee 0.8 extension: domain_check build');
is($dri->get_info('action'),'check','fee 0.8 extension: domain_check get_info(action)');
is($dri->get_info('exist'),0,'fee 0.8 extension: domain_check get_info(exist)');
$d = shift @{$dri->get_info('fee')};
is($d->{domain},'crrc.in','fee 0.8 extension: domain_check single parse domain');
is($d->{premium},1,'fee 0.8 extension: domain_check single parse premium');
is($d->{currency},'INR','fee 0.8 extension: domain_check single parse currency');
is($d->{action},'create','fee 0.8 extension: domain_check single parse action');
is($d->{duration}->years(),1,'fee 0.8 extension: domain_check singe parse duration');
is($d->{fee},100.00,'fee 0.8 extension: domain_check singe parse fee');

## fee-create
$R2=$E1.'<response>'.r().'<resData><domain:creData xmlns:domain="urn:ietf:params:xml:ns:domain-1.0"><domain:name>crrc.in</domain:name><domain:crDate>2016-04-11T07:31:58.0Z</domain:crDate><domain:exDate>2017-04-11T07:31:58.0Z</domain:exDate></domain:creData></resData><extension><fee:creData xmlns:fee="urn:ietf:params:xml:ns:fee-0.8"><fee:currency>INR</fee:currency><fee:fee applied="delayed" grace-period="P8D" refundable="1">100.00</fee:fee><fee:balance>9664204.00</fee:balance></fee:creData></extension>'.$TRID.'</response>'.$E2;
$cs=$dri->local_object('contactset');
...
is_string($R1,$E1.'<command><create><domain:create xmlns:domain="urn:ietf:params:xml:ns:domain-1.0" xsi:schemaLocation="urn:ietf:params:xml:ns:domain-1.0 domain-1.0.xsd"><domain:name>crrc.in</domain:name><domain:period unit="y">1</domain:period><domain:registrant>abc001</domain:registrant><domain:contact type="admin">abc001</domain:contact><domain:contact type="billing">abc001</domain:contact><domain:contact type="tech">abc001</domain:contact><domain:authInfo><domain:pw>abc123</domain:pw></domain:authInfo></domain:create></create><extension><fee:create xmlns:fee="urn:ietf:params:xml:ns:fee-0.8" xsi:schemaLocation="urn:ietf:params:xml:ns:fee-0.8 fee-0.8.xsd"><fee:currency>INR</fee:currency><fee:fee>100.00</fee:fee></fee:create></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'fee 0.8 extension: domain_create build');
is($rc->is_success(),1,'fee 0.8 extension: domain_create is is_success');
is($dri->get_info('action'),'create','fee 0.8 extension: domain_create get_info (action)');
$d=$rc->get_data('fee');
is($d->{currency},'INR','fee 0.8 extension: domain_create parse currency');
is($d->{fee},100.00,'fee 0.8 extension: domain_create parse fee');
is($d->{balance},9664204.00,'fee 0.8 extension: domain_create parse balance');
