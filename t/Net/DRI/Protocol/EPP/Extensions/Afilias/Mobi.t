...
$cs->set($c2,'tech');
$cs->set($c2,'billing');
$rc=$dri->domain_create('whatever.eco',{pure_create=>1,duration=>DateTime::Duration->new(years=>2),ns=>$dri->local_object('hosts')->set(['ns1.example.com'],['ns1.example.net']),contact=>$cs,auth=>{pw=>'2fooBAR'},maintainer_url=>'http://www.afilias.info'});
...
$rc=$dri->domain_update('whatever.eco',$toc);
...
$rc=$dri->domain_info('whatever.eco',{auth=>{pw=>'2fooBAR'}});
