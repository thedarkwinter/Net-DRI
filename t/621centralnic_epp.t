...
$rc=$dri->domain_check('example.cafe','example.bar','example.wiki');
is($rc->is_success(),1,'domain_check multi is_success');
is($dri->get_info('action'),'check','domain_check multi get_info (action)');
is($dri->get_info('exist','domain','example.cafe'),0,'domain_check multi get_info(exist) 1/2');
is($dri->get_info('exist','domain','example.bar'),1,'domain_check multi get_info(exist) 2/3');
is($dri->get_info('exist','domain','example.wiki'),1,'domain_check multi get_info(exist) 3/3');
$d = shift $dri->get_info('fee','domain','example.cafe');
is($d->{domain},'example.cafe','domain_check multi get_info(domain) 1/3');
...
$d = shift $dri->get_info('fee','domain','example.bar');
is($d->{domain},'example.bar','domain_check multi get_info(domain) 2/3');
...
$d = shift $dri->get_info('fee','domain','example.wiki');
is($d->{domain},'example.wiki','domain_check multi get_info(domain) 3/3');
...
is_string($R1,$E1.'<command><transfer op="request"><domain:transfer xmlns:domain="urn:ietf:params:xml:ns:domain-1.0" xsi:schemaLocation="urn:ietf:params:xml:ns:domain-1.0 domain-1.0.xsd"><domain:name>example.cafe</domain:name><domain:period unit="y">1</domain:period><domain:authInfo><domain:pw roid="JD1234-REP">2fooBAR</domain:pw></domain:authInfo></domain:transfer></transfer><extension><fee:create xmlns:fee="urn:ietf:params:xml:ns:fee-0.4" xsi:schemaLocation="urn:ietf:params:xml:ns:fee-0.4 fee-0.4.xsd"><fee:currency>USD</fee:currency><fee:fee>5.00</fee:fee></fee:create></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'Fee extension: domain_transfer build');
...
is_string($R1,$E1.'<command><update><domain:update xmlns:domain="urn:ietf:params:xml:ns:domain-1.0" xsi:schemaLocation="urn:ietf:params:xml:ns:domain-1.0 domain-1.0.xsd"><domain:name>example.cafe</domain:name><domain:chg><domain:registrant>sh8013</domain:registrant></domain:chg></domain:update></update><extension><fee:create xmlns:fee="urn:ietf:params:xml:ns:fee-0.4" xsi:schemaLocation="urn:ietf:params:xml:ns:fee-0.4 fee-0.4.xsd"><fee:currency>USD</fee:currency><fee:fee>5.00</fee:fee></fee:create></extension><clTRID>ABC-12345</clTRID></command>'.$E2,'Fee extension: domain_update build');
