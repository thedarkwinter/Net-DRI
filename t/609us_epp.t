...
sub SUB    { our $inc=$inc+1; my $i = $inc; if ($i < 10) { $i = "0".$i; } return 'nom-iq'.$i; };
sub SUB1   { my $i = $inc; if ($i < 10) { $i = "0".$i; } return $i; };

my ($rc, $tnmbr, $co, $s);
my $dri=Net::DRI::TrapExceptions->new({cache_ttl=>10,trid_factory => sub { return 'nom-iq'.&SUB1().'ote-testcase'.&SUB1().'cmd'}});
$dri->add_registry('US');
$dri->target('US')->add_current_profile('p1','epp',{f_send=>\&mysend,f_recv=>\&myrecv});
my $drd = $dri->{registries}->{US}->{driver};
is_deeply( [$drd->transport_protocol_default('epp')],['Net::DRI::Transport::Socket',{},'Net::DRI::Protocol::EPP::Extensions::US',{}],'US - epp transport_protocol_default');
$R2='';
...
is($R1,$E1.'<hello/>'.$E2,'session noop build (Test 1)');
is($rc->is_success(),1,'session noop is_success (Test 1)');
